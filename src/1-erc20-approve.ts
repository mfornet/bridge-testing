...
async function main() {
    ...
    console.log(
        "Approving to Locker contract transfer of",
    ...
    console.log(
        "Initial approved amount of tokens:",
        initialApprovedAmount.toString()
    );
    if (initialApprovedAmount >= depositAmount) {
        console.log(
            "There's already sufficient amount of approved tokens, no need to schedule a transaction"
        );
        return;
    }
    ...
    const approveTx = await erc20.approve(
        ethereumConfig.ConnectorAddress,
        ethers.BigNumber.from(ethereumConfig.DepositAmount)
    );
    ...
    const finalApprovedAmount = new BN(
        (
            await erc20.allowance(
                ethereumConfig.Address,
                ethereumConfig.ConnectorAddress
    ...
    console.log(
        "Resulting approved amount of tokens:",
        finalApprovedAmount.toString()
    );
}
